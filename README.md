...
## PURPOSE OF SOFTWARE: Matrix library that solves: D = A + (3 * B) × C^T with given matrices. 
## FILES 
...
## BUILD INSTRUCTIONS ``` make all ``` 
## RUNNING THE CODE ``` ./cpp_test ./c_test ``` 
## TESTING METHODOLOGY: Both implementations calculate D = A + (3 * B) × C^T step by step. 
